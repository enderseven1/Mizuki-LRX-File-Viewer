...
if __name__ == '__main__':
    ...
    bantext = ['"', '?', '>', '<', '\\', '|', '*', '/', ' ']
    strings = {#'':{'中文':'','English':''},
               'BrokenEnding':{'中文':'错误编辑或其他原因导致尾信息失效，若读取的文本异常，可能该文件已损坏。（修改并保存后自动修复尾信息）',
                               'English':'Bad editing or other reasons that cause the end of the file to become corrupted. If the text read is abnormal, the file may be corrupted. (Automatically fix the end of the file after modifying and saving)'},
               'ContentRead':{'中文':'已成功打开文件，单击OK即可保存。','English':'The file was successfully opened. Click OK to save.'},
               'ContentWrited':{'中文':'内容已完整写入文件中，内容如下：','English':'Content written: '},
               'Copyright':{'中文':'\n感谢使用。\ncopyright©2023 Shimadamizuki','English':'\nThanks for using.\ncopyright©2023 Shimadamizuki'},
               'CopyrightTitle':{'中文':'版权信息','English':'Copyright Information'},
               'CreateFile1':{'中文':'是否要在当前位置（','English':'Do you want to create a new file at '},
               'CreateFile2':{'中文':'\\）创建文件？','English':'\\?'},
               ...
               'InvalidFile':{'中文':'无效lrx文件（文件头无效）','English':'Invalid lrx file (Invalid heading).'},
               'LRXError1':{'中文':'梁如萱不爱我...o(╥﹏╥)o','English':'Liang Sharon (Ruxuan) don\'t love me...o(╥﹏╥)o'},
               ...
               'Selver':{'中文':'选择版本','English':'Version select'},
               'SetFilenameTitle':{'中文':'文件名','English':'Set a filename'},
               'Tips':{'中文':'提示信息','English':'Message'},
               ...
               'Whichver':{'中文':'请选择文件版本','English':'Please select a version for new file.'},
               'WriteDoneLog':{'中文':'保存成功','English':'Saved successfully.'},
    ...
    def readfile(filename):
        ...
        lrx = open(filename,'a+', encoding='utf-8')
        lrxcontt = ''
        lrx.seek(0)
        ...
        if lrxhead == 'Heading: ' + heads['1.0'] or lrxhead == 'Heading: ' + heads['2.0'] or lrxhead == 'Heading: ' + heads['3.0'] or lrxhead == 'Heading: ' + heads['3.1']:
            ...
            if editend == contents or editend == None:
                ...
                if '梁如萱我爱你' in editend or 'Liang Ruxuan I Love You'.lower() in editend.lower():
    ...
    if language != None:
        ...
        while True:
            syss = easygui.buttonbox(msg=strings['Welcome'][language] + '\n\n' +strings['PlsSelectAction'][language], title=strings['WelcomeTitle'][language], choices=(strings['Select'][language], strings['NewFile'][language]))
            if syss == strings['Select'][language]:
                ...
                if isnew == strings['Yes'][language]:
                    while True:
                        whichver = easygui.buttonbox(msg=strings['Whichver'][language], title=strings['Selver'][language], choices=('1.0', '2.0', '3.0', '3.1'))
                        if whichver != None:
                            while True:
        ...
        #easygui.msgbox(strings['Copyright'][language], strings['CopyrightTitle'][language])
