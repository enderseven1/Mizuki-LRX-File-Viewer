...
OLD_VERS = ('1.5.2', '1.5.3', '1.5.4') # 旧版本 Old versions.
VERSION = '1.5.5' # 当前版本 Current version.
WHAT_S_NEW = {'中文':('感谢你更新到 v' + VERSION + ' 版本！以下是该版本的最新特性：\n'
                    '一、改进密码记录方式。\n二、改进代码运行顺序。\n三、可以随时查看更新信息了。'),
              'English':('Thank you for update to version ' + VERSION + '!\nWhat\'s new:\n'
                         '1. Improve the password system.\n'
                         '2. Improve code run order.\n'
                         '3. You can view the update information at any time.')} # 更新内容 What's New.
...
def lrxdecode32(content_of_lrxfile, password_of_file=None):
    '''
        3.2解码器。
    '''
    global password
...
def readfile(filename, password_of_file=None):
    '''
        读取文件。
    '''
    ...
    try:
        with open(filename,'a+', encoding='utf-8') as lrx: # 打开文件
            edited_content = reads(filename,lrx,password_of_file)
...
def reads(lrxfilename,fileobj,password_of_file=None):
    '''
        读取文件内容。
        lrxfilename：文件路径
        fileobj：已打开的文件对象
    ...
    if header_checked == 'Unsupported':
        ...
        if version_of_file == '3.2': # 解码
            while True:
                processed_content = lrxdecode32(unprocessed_content, password_of_file)
