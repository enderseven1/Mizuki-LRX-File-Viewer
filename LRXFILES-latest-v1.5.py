...
strings = {'中文': {'BrokenEnding': ('错误编辑等原因破坏了尾信息，若读取的文本异常，可能该文件已损坏。' #定义语言
                    ...
                    'Continue': '继续',
                    'CopyrightTitle': '感谢使用', 
                    'CreateFile1': '是否要在当前位置（', 
                    'CreateFile2': '\\）创建文件？', 
                    'DelLang': '切换语言', 
                    'EnterFilename': '请输入你的文件名（自动添加.lrx扩展名）', 
                    'EnterName': '输入文件名', 
                    'ErrorTip': '错误提示', 
                    'FileCreated': '文件已创建：', 
                    'FileExists': '文件已存在，请重新输入。', 
                    'FilenameSetWays': '文件名要怎样设置？', 
            ...
            'English': {'BrokenEnding': ('Bad editing or other reasons that corrupted the file ending. '
                    ...
                    'Continue': 'Continue',
                    'CopyrightTitle': 'Thanks for Using.', 
                    'CreateFile1': 'Do you want to create a new file at ', 
                    'CreateFile2': '\\?', 
                    'DelLang': 'Change Language', 
                    'EnterFilename': 'Please enter your file name (without the extension)', 
                    'EnterName': 'Enter a filename', 
                    'ErrorTip': 'Error', 
                    'FileCreated': 'File created at ', 
                    'FileExists': 'The file already exists, please re-enter a filename.', 
                    'FilenameSetWays': 'How to set the filename?', 
...
def checkLangConfig():
    ...
    if os.path.exists(logfile):
        with open(logfile, 'r', encoding='utf-8') as log:
            loged = log.readlines()
            for lines in loged:
                if lines.startswith('Language: '):
                    kkk = lines.replace('Language: ','')
                    if kkk in strings:
                        language = kkk
                        return True
...
if __name__ == '__main__':
    while True:
        ...
        if language != None:
            ...
            while True:
                syss = easygui.buttonbox(msg=strings[language]['Welcome'] + '\n\n' +strings[language]['PlsSelectAction'], title=strings[language]['WelcomeTitle'], choices=(strings[language]['Select'], strings[language]['NewFile'], strings[language]['DelLang'])) #syss is selection of this window to select an action. *syss 是用户在此窗口选择的项，用于获取用户的操作。
                if syss == strings[language]['Select']: #If "Select a file" is selected. *如果选择“选择文件”。
                    ...
                    isnew = easygui.buttonbox(msg=strings[language]['CreateFile1'] + os.getcwd() + strings[language]['CreateFile2'], title=strings[language]['NewFile'], choices=(strings[language]['Yes'], strings[language]['No'])) #isnew is selection of this window to confirm whether a new file is created in the current location. *isnew 是用户在此窗口选择的项，用于确认是否在当前位置新建文件。
                    if isnew == strings[language]['Yes']: #Yes *选是
                        while True:
                            whichver = easygui.buttonbox(msg=strings[language]['Whichver'], title=strings[language]['Selver'], choices=('1.0', '2.0', '3.0', '3.1')) #whichver is version is selected of file. *whichver是选择的文件版本
                            if whichver != None:
                                while True:
                                    isname = easygui.buttonbox(msg=strings[language]['FilenameSetWays'], title=strings[language]['SetFilenameTitle'], choices=(strings[language]['Self'], strings[language]['Random']))
                                    if isname != None:
                                        if isname == strings[language]['Self']: #自己输入文件名
                                            while True:
                                                newf = easygui.enterbox(msg=strings[language]['EnterFilename'], title=strings[language]['EnterName'])
                                                if newf != None:
                                                    newf += '.lrx'
                                                    ...
                                                    break
                                        elif isname == strings[language]['Random']: #随机生成文件名
                                            while True:
                                                newf = ''
                    ...
                    break
            if syss == strings[language]['DelLang']:
                continue
